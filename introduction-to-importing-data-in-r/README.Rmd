...
This course focuses on importing 5 types of data:
...
Structure corresponds to data frame.

R makes importing it as so easy.

``` r
# file in ~/datasets/states.csv
path <- file.path("~", "datasets", "states.csv")  # OS ignostic
read.csv(path, stringsAsFactors = FALSE)
```

`stringAsFactors` TRUE by default, determines whether to convert strings to factors for categorical data.

In the example `states.csv` file the state and capital names should not be categorical variables.

### read.csv

The `utils` package, which is automatically loaded in your R session on startup, can import CSV files with the `read.csv()` function.

In this exercise, you'll be working with `swimming_pools.csv`; it contains data on swimming pools in Brisbane, Australia (Source: [data.gov.au](https://data.gov.au/dataset/swimming-pools-brisbane-city-council)). The file contains the column names in the first row. It uses a comma to separate values within rows.
...
```{r}
path <- file.path("data", "swimming_pools.csv")  # relative

pools <- read.csv(path, stringsAsFactors = TRUE)  # FALSE is the default as of R 4.0.0
...
**NOTE:** in R version 4.0.0 the default value of `stringsAsFactors` was changed to `FALSE`. [source](https://github.com/thomas-neitmann/past)
...
pools <- read.csv(path, stringsAsFactors = FALSE)  # FALSE is the default as of R 4.0.0
...
Aside from `.csv` files, there are also the `.txt` files which are basically text files. You can import these ~~functions~~ files with `read.delim()`. By default, it sets the `sep` argument to `"\t"` (fields in a record are delimited by tabs) and the `header` argument to `TRUE` (the first row contains the field names).
...
hotdogs <- read.delim(path, header = FALSE)
...
`sep = ""` means the separator is "white space", that is one or more spaces, tabs, newlines, or carriage returns.
...
`read.csv()` and `read.tsv()` are wrappers around `read.table()`.
...
Until now we've been using the `utils` package that is loaded by default when you start R.

There are also specialized packages such as `rear` and `data.table`
...
``` r
read.csv("states.csv", stringsAsFactors = False)  # FALSE is the default as of 4.0.0
```
...
`read_csv()` outputs a *tibble* - a super-charged data frame introduced by Hadley Wickham.

in `readr` strings are not imported as factors by default.
...
As in `utils`, `read_csv()` and `read_tsv()` are wrappers around `read_delim()`.
...
-   `read_delim2()` reads in files with any delimiter, attempting to automatically guess the delimiter if you don’t specify it using "," as the decimal marker.
...
path <- file.path("data", "potatoes.csv")
...
This time, the potatoes data comes in the form of a tab-separated values file; potatoes.tsv is available in your workspace. In contrast to `potatoes.csv`, this file does **not** contain columns names in the first row, though.

There's a vector `properties` that you can use to specify these column names manually.

##### Instructions

-   Use `read_tsv()` to import the potatoes data from `potatoes.tsv` and store it in the data frame `potatoes.` In addition to the path to the file, you'll also have to specify the `col_names` argument; you can use the `properties` vector for this.
...
path <- file.path("data", "potatoes.tsv")
...
You can explicitly set the column types.
...
Skipped two rows (row 3, New York, was used as the header) and read in a maximum of three rows. To fix:
...
You'll again be working with `potatoes.tsv`; the file uses tabs (`"\t"`) to delimit values and does **not** contain column names in its first line. It's available in your working directory so you can start right away. As before, the vector `properties` is available to set the `col_names`.
...
path <- file.path("data", "potatoes.tsv")
potatoes <- read_delim(path, "\t", col_names = properties)
...
For a complete list of collector functions, you can take a look at the [`collector`](https://www.rdocumentation.org/packages/readr/versions/0.2.2/topics/collector) documentation. For this exercise you will need two collector functions:
...
-   key metric: speed
-   primarily for data manipulation in R
-   includes `fread()`

If you have huge files you want to import and munge in R, you'll want to use `data.table`.

``` r
install.packages("data.tab;e")
library("data.table")
```

### fread()

states.csv (with header):
...
-   in the absence of column names, `fread()` will provide them.
-   can infer column types and separators
-   simply works!
-   extremely fast
-   possibly to specify:
    -   separator
...
Faster, more convenient version of `read.table()` with more functionality.
...
# use pacman::p_load to install and load data.table (and/or others) if not already available
...
[source](https://stackoverflow.com/questions/4090169/elegant-way-to-check-for-missing-packages-and-install-them/19873732#19873732)

[Introduction to pacman](https://trinker.github.io/pacman/vignettes/Introduction_to_pacman.html)
...
path <- file.path("data", "potatoes.csv")
# potatoes <- fread(path, drop = c(1:5, 7))
# potatoes <- fread(path, select = c(6, 8))
# potatoes <- fread(path, drop = c("area", "temp", "size", "storage", "method", "flavor"))
potatoes <- fread(path, select = c("texture", "moistness"))

# Plot texture (x) and moistness (y) of potatoes
plot(x = potatoes$texture, y = potatoes$moistness, main = "Potato Texture vs Moistness", xlab = "Texture", ylab = "Moistness", pch = 19, col = "brown")
grid()
```
...
-   Hadley Wickham
-   `excel_sheeets()` - lists different sheets
-   `read_excel()` - imports data into R

```{r}
pacman::p_load("readxl")

path <- file.path("data", "urbanpop.xlsx")
...
read_excel(path, sheet = "1975-2011")
```
...
pop_2 <- read_excel(path, sheet = 2)
pop_3 <- read_excel(path, sheet = 3)
...
pop_list <- lapply(excel_sheets(path),
                   read_excel,
                   path = path)
...
-   `col_names = TRUE`: first row of the worksheet holds the column names
-   `col_names = FALSE`: R assigns names itself
-   `col_names = character vector`: manually specify

#### col_types

-   `col_types = NULL`: R guesses the data types
-   `col_types = character vector`: manually specify
-   `col_types = numeric`: specify column is numeric
-   `col_types = date`: specify column is a date
-   `col_types = blank`: column is ignored
...
path <- file.path("data", "urbanpop_nonames.xlsx")

# Import the first Excel sheet of urbanpop_nonames.xlsx (R gives names): pop_a
pop_a <- read_excel(path, col_names = FALSE)

# Import the first Excel sheet of urbanpop_nonames.xlsx (specify col_names): pop_b
cols <- c("country", paste0("year_", 1960:1966))
pop_b <- read_excel(path, col_names = cols)
...
urbanpop_sel <- read_excel(path, col_names = FALSE, sheet = 2, skip = 21)
...
* `read.xls`
* Support for XLS only out-of-box
...
urban_pop <- read.xls(path, sheet = 2)

head(urban_pop, n = 11)
```
**Note:** Excel file support is deprecated in gdata 2.19.0 and was be removed in version 3.0.0.

[source](https://rdocumentation.org/packages/gdata/versions/2.19.0)
...
* Martin Studer
* Word with Excel through R
* Bridge between Excel and R
* XLS and XLSX

XLConnect functions expose all(?) Excel functionality.

Depends on Java. Will to install the `rJava` package. You may need to install the Java JDK yourself.
...
str(book)
getSheets(book)
df <- readWorksheet(book, sheet = "1960-1966", startRow = 10, endRow = 11, startCol = 1, endCol = 4, header = TRUE)
df
```
...
With XLConnect you can programmatically interact with Excel.

```r
book <- loadWorkbook(path)
createSheet(book, name = sheet_name)
writeWorksheet(book, data_to_be_added, sheet_name)
saveWorkbook(book, file = file_name)  # write to a new file for safety?
```

### renameSheet()

Allows for renaming worksheets.

```r
renameSheet(book, old_name, new_name)
saveWorkbook(book, file = file_name)  # new file for safety
```

### removeSheet()

```r
removeSheet(book, sheet = sheet_name)
removeSheet(book, sheet = sheet_number)
saveWorkbook(book, file = file_name)  # new file for safety
```
...
`book` is an in-memory representation of the Excel workbook.
...
read_path <- file.path("data", "urbanpop.xlsx")
write_path <- file.path("data", "summary.xlsx")

my_book <- loadWorkbook(read_path)
...
saveWorkbook(my_book, file = write_path)
```
...
read_path <- file.path("data", "summary.xlsx")
write_path <- file.path("data", "renamed.xlsx")
...
read_path <- file.path("data", "renamed.xlsx")
write_path <- file.path("data", "clean.xlsx")
